...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Recibie datos de la Factura
        $num_factura = $_POST['num_factura'] ?? '';
        $fecha_factura = $_POST['fecha_factura'] ?? '';
        $metodo_pago = $_POST['metodo_pago'] ?? '';
        $moneda = $_POST['moneda'] ?? '';
        
        // Recibie datos de Exportación
        $fecha = $_POST['fecha'] ?? '';
        ...
        // PASO B: Insertar en la tabla Exportacion usando el id_factura (Foreign Key)
        $sqlImpo = "INSERT INTO Exportacion (id_factura, fecha, cantidad, unidad_medida, pais, tipo_cambio, valor_dolares, costo_unitario, tipo_impo, peso_neto, descripcion, peso_bruto, total) 
        ...
        // CONFIRMAMOS AMBOS REGISTROS
        $conexion->commit();

        echo "<script>alert('Factura e Exportacion guardadas exitosamente.'); window.location.href='exportacion.html';</script>";
    }
} catch (PDOException $e) {
    // Si hay algún error, deshace todo para no dejar registros 
    if (isset($conexion)) {
        $conexion->rollBack();
    }
